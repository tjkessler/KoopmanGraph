...
- Documented [uv](https://docs.astral.sh/uv/) install paths alongside pip
  ...
  ``astral-sh/setup-uv``.
- Speed up CI: path-filtered jobs, separate Ruff/`uvx` lint job, drop the
  Python 3.11 matrix leg, parallelize pytest with ``-n auto``, collect coverage
  only on 3.12, use ``uv sync --frozen`` / ``uv run --no-sync``, and require the
  aggregator ``ci`` check instead of every leaf job.

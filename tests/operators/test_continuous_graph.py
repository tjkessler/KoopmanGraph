...
def test_continuous_spectrum_ineligible_skips_kronecker(
    ...
    """Dual continuous spectrum dense-routes; distributed uses Arnoldi.

    Orbit / isotypic banks are N/A on continuous graph. Dual adjacency is
    Kronecker-ineligible and uses dense :math:`L_{\\mathrm{eff}}`.
    Distributed spectrum is a matrix-free Arnoldi surrogate (placeholder
    eigenvectors), not a certified continuous eigendecomposition.
    """
    from koopman_graph.operators import continuous_graph as cont_mod
    ...
    ambient = 6
    assert dist_spec.eigenvalues.numel() <= ambient
    assert dist_spec.eigenvalues.numel() >= 1
    assert dist_spec.eigenvectors.shape[0] == dist_spec.eigenvalues.numel()

...
def test_console_script_version() -> None:
    ...
    repo_script = REPO_ROOT / ".venv" / "bin" / "koopman-graph"
    exe_script = Path(sys.executable).resolve().parent / "koopman-graph"
    script = repo_script if repo_script.exists() else exe_script
    result = subprocess.run(
        [str(script), "--version"],
        check=False,
        capture_output=True,

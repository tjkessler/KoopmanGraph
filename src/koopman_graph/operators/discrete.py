    ...
    **Structural (eigenvalues forced strictly inside the unit disk):**
    ...
    def __init__(
        ...
        *,
        ...
        # Ephemeral assembled-K reuse for structural modes (keyed by factor
        # ``_version``); never written to ``state_dict``.
        self._assembled_k_cache: Tensor | None = None
        self._assembled_k_cache_key: tuple[object, ...] | None = None
    ...
    def _structural_assembly_factors(self) -> tuple[Tensor, ...]:
        """Return learnable tensors that feed non-dense ``K`` assembly.
    ...
    def _assembled_k_key(self) -> tuple[object, ...]:
        """Build the cache key for structural ``K``.
        ...
        return (
            ...
            float(self.max_spectral_radius),
    ...
    def K(self) -> Tensor:
        ...
        Other modes assemble ``K`` from their factorized parameters and may
        ...
        if self.parameterization == "dense":
            ...
            return dense_k
        key = self._assembled_k_key()
        if self._assembled_k_cache is not None and self._assembled_k_cache_key == key:
            return self._assembled_k_cache
        matrix = self._assemble_matrix()
        self._assembled_k_cache = matrix
        self._assembled_k_cache_key = key
        return matrix

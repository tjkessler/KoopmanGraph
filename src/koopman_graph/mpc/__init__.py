...
Scope (0.6.0): discrete per-node ``KoopmanOperator`` with
``control_mode="additive"`` only. Networked / continuous / bilinear
operators are rejected. Output-constraint guarantees are local (decoder
Jacobian linearization). Optional ``constraint_tightening=`` accepts a
calibrated :class:`~koopman_graph.uq.ConformalKoopmanUQ` and shrinks
output boxes by per-horizon half-widths (inputs unchanged; margins are
calibrated, not a formal closed-loop guarantee).

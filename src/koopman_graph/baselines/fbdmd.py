"""Forward–backward Dynamic Mode Decomposition baseline.

Fits forward and backward least-squares maps on flattened node states and
returns a matrix square root of their composition (forward–backward DMD).
...
class FBDMDBaseline(ClassicalBaseline):
    """Forward–backward DMD baseline on flattened node states.

    Ignores graph message passing: each snapshot is reshaped into one vector
    and a forward–backward linear map is fit. The learned operator follows
    ``x_next = x @ K.T``.
    ...
    rank : int or None or {"auto"}, optional
        Truncated-SVD rank for each directional fit. ``None`` uses full
        least squares. ``"auto"`` selects the Gavish–Donoho unknown-``σ``
        median hard threshold. Default is ``None``.
    """

    def _is_fitted(self) -> bool:
        """Return whether the forward–backward operator has been fit.
    ...
    def fit(
        ...
        """Fit the forward–backward DMD operator from consecutive snapshots.
    ...
    def spectrum(self) -> KoopmanSpectrum:
        """Return the forward–backward DMD operator spectrum.

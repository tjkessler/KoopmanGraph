...
Each forecast step follows an encode--linear advance--decode workflow: GCN or GAT encoders lift node features into a latent space with message passing on the graph; a learnable finite-dimensional Koopman operator propagates latent states in discrete time or via a continuous-time generator; and a matching GNN decoder maps predictions back to physical node features. The `GraphKoopmanModel` class exposes `fit`, `predict`, and `evaluate` methods for training on sequences of PyTorch Geometric `Data` snapshots, rolling out multi-step forecasts, and reporting per-horizon error metrics. Structural stability constraints, spectral analysis, classical DMD/EDMD baselines, Koopman-with-control dynamics, online adaptation, and dynamic-topology sequences are supported for interpretation, comparison, and domain-specific experimentation. The software is available at \url{https://github.com/tjkessler/KoopmanGraph}.
...
`KoopmanGraph` was developed as a standalone package rather than a patch to existing Koopman tools because graph topology must enter at the encoder and decoder stages---not only in the loss---through shared `edge_index` message passing. The design keeps PyG as the geometric backbone while exposing a small, stable public API (`GraphKoopmanModel`, `GNNEncoder`, `GATEncoder`, `GNNDecoder`, `GATDecoder`, `KoopmanOperator`, `ContinuousKoopmanOperator`) aligned with Koopman autoencoder practice [@Mezic2021; @Lusch2018].

# Software design

`KoopmanGraph` follows a modular three-stage architecture for each forecast step:

1. **Lifting (encoder).** `GNNEncoder` (GCN) or `GATEncoder` (GAT) maps node features $\mathbf{x}_t \in \mathbb{R}^{N \times F}$ to latent node states $\mathbf{z}_t \in \mathbb{R}^{N \times d}$ using the graph's `edge_index`. Optional hybrid observables can concatenate known structural features (for example graph Laplacian coordinates) with learned latents.
2. **Linear evolution (Koopman operator).** Discrete `KoopmanOperator` applies a learnable matrix $\mathbf{K} \in \mathbb{R}^{d \times d}$ via $\mathbf{z}_{t+1} \approx \mathbf{z}_t \mathbf{K}^\top$ at each node. Continuous-time `ContinuousKoopmanOperator` instead learns a generator integrated with matrix exponentials, supporting irregular sampling through `predict_at`.
...
`GraphSnapshotSequence` validates that all timesteps share a consistent topology (with an opt-in dynamic-topology mode for rewiring graphs) and provides helpers to build sequences from NumPy or PyTorch arrays, optionally carrying shared `edge_weight` tensors and per-timestep control inputs. Explicit `MultiTrajectory` wrappers support multi-sequence fitting. Training via `GraphKoopmanModel.fit` minimizes a weighted sum of reconstruction, forward consistency, backward consistency, rollout, and optional eigenvalue regularization losses. Optional gradient clipping, loss-weight schedules, validation-aware early stopping, learning-rate schedulers, multi-trajectory aggregation, and windowed mini-batching improve stability and scalability on longer sequences. `GraphKoopmanModel.predict` performs autoregressive rollout in eval mode and returns a list of PyG `Data` snapshots; `GraphKoopmanModel.evaluate` reports per-horizon MAE, RMSE, and MAPE on held-out data. `GraphKoopmanModel.save` and `GraphKoopmanModel.load` persist trained weights together with encoder/decoder architecture configuration. `model.spectrum()` exposes eigendecomposition of the learned operator with continuous-time growth rates and frequencies derived from `time_step`, and `decode_mode_shapes` maps latent Koopman modes back to node-space patterns. Online recursive least-squares adaptation can update the Koopman operator after deployment while keeping the GNN encoder frozen, and `GraphKoopmanEnv` wraps the trained model as a Gymnasium environment for latent-space closed-loop control.

The package is distributed on PyPI as `koopman-graph` [@koopmangraph2026], requires Python 3.10+, PyTorch, and PyTorch Geometric, and includes Sphinx documentation hosted on Read the Docs. The public API emphasizes a thin root façade for core workflow symbols, with specialized helpers available from capability modules such as `metrics`, `analysis`, `data`, `adaptation`, and `observables`.

# Features

At the time of writing, `KoopmanGraph` provides the following functionality:

- **End-to-end model.** `GraphKoopmanModel` composes encoder, Koopman operator, and decoder with `fit`, `predict`, `evaluate`, `save`, `load`, and `spectrum` APIs, including optional operator injection for composition.
- **Topology-aware encoders and decoders.** Symmetric GCN (`GNNEncoder` / `GNNDecoder`) and GAT (`GATEncoder` / `GATDecoder`) pairs for latent lifting and reconstruction with message passing on `edge_index`; GCN paths propagate optional scalar `edge_weight`.
- **Learnable linear dynamics.** Discrete `KoopmanOperator` and continuous-time `ContinuousKoopmanOperator` with configurable matrix initialization (`identity`, `identity_noise`, `xavier`). Soft spectral constraints (orthogonal--diagonal--orthogonal) and structural parameterizations (`schur`, `dissipative`, `lyapunov`) support long-horizon stability; continuous mode uses matrix-exponential integration and `predict_at` for irregular sampling.
- **Koopman with control.** Optional input matrix `B` and per-timestep control sequences for driven dynamics (`z_{t+1} = K z_t + B u_t`).
- **Online adaptation.** `RecursiveKoopmanAdapter` updates the Koopman operator online with a frozen encoder under distributional shift.
...
- **Spectral analysis and similarity.** `KoopmanSpectrum`, `compute_spectrum`, and `decode_mode_shapes` for eigenvalues, mode amplitudes, continuous-time frequencies, and spatial mode shapes; `spectrum_distance`, `koopman_std`, and `detect_anomaly` for regime comparison and anomaly detection.
- **Physics-informed observables.** Hybrid lifting that concatenates known structural features with learned latents.
- **Sequence utilities.** `GraphSnapshotSequence` for validated, time-ordered PyG snapshot containers; `MultiTrajectory` for explicit multi-sequence fitting; `TemporalSplit` / `temporal_split` for train/validation/test partitioning; `WindowSampler` for fixed-length mini-batch training.
...
- **Latent-space RL.** `GraphKoopmanEnv` Gymnasium wrapper for closed-loop control experiments on trained models.
...
- **Tutorials and tests.** Sixteen Jupyter notebooks, Sphinx API reference, pytest suite with an enforced 95% coverage gate, and nbmake smoke tests in CI.
...
`KoopmanGraph` provides a central, PyG-native toolkit for topology-aware Koopman modeling of spatiotemporal graph dynamics. By combining GNN encoders and decoders with discrete and continuous-time Koopman operators, structural stability options, consistency losses, spectral analysis, classical baselines, online adaptation, and evaluation utilities, the library makes it straightforward to train, evaluate, and interpret graph-respecting forecasting models that retain explicit linear latent dynamics. We hope that researchers and practitioners will use `KoopmanGraph` as a platform for experimentation on networked dynamical systems and for extending Koopman-theoretic methods to graph-structured data.

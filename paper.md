...
**v0.3.0 additions** extend the library with guaranteed long-horizon stability via structurally constrained Koopman parameterizations (`schur`, `dissipative`, `lyapunov`); continuous-time generator learning with matrix-exponential integration and `predict_at` for irregular sampling; recursive least-squares online adaptation of the Koopman operator with a frozen GNN encoder; hybrid physics-informed observables that concatenate known structural features with learned latents; dynamical similarity metrics (`spectrum_distance`, `koopman_std`, `detect_anomaly`) for regime comparison; and a Gymnasium environment wrapper (`GraphKoopmanEnv`) for latent-space reinforcement learning. The same release also standardizes the public API surface: symmetric GCN/GAT encoder–decoder pairs, a preferred `encode` method, explicit `MultiTrajectory` multi-sequence fitting, shared autoregressive rollout primitives (inference hold-last vs training teacher-target topology), optional `koopman=` operator injection for composition, frozen public result dataclasses, classical baseline scaffolding with an explicit `ForecastModel` call-site contract (autonomous peers vs control-required `DMDcBaseline`), shared discrete/continuous propagation helpers, a thin root façade (core workflow symbols in `__all__`; specialized helpers via `metrics` / `analysis` / `data` / `adaptation` / `observables`), and documented power-user modules (`graph_utils`, `protocols`) that are importable but not re-exported from the top-level package.

...
2. **Linear evolution.** Discrete `KoopmanOperator` applies $\mathbf{K} \in \mathbb{R}^{d \times d}$ via $\mathbf{z}_{t+1} \approx \mathbf{z}_t \mathbf{K}^\top$. Optional `GraphKoopmanOperator` (`koopman="graph"`) uses a factorized self/neighbor form inspired by compositional / networked Koopman constructions [@Li2020CompositionalKoopman; @Mukherjee2022]; that factorization is a parameter-tying inductive bias, not a claim of demonstrated cross-topology zero-shot superiority. Continuous `ContinuousKoopmanOperator` learns a matrix-exponential generator with irregular-time `predict_at`.
